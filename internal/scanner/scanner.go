...
func Scan(ctx context.Context, opts ScanOptions) (*models.ScanResult, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("%w: cannot access root path: %v", errScanResultValidation, err)
	}
	if !info.IsDir() {
	...
	if err != nil {
		return nil, fmt.Errorf("%w: cannot get absolute path: %v", errScanResultValidation, err)
	}
